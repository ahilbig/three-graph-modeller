...
export class PerspectiveCamera extends Camera {
     ...
     *     // F
     *     camera.setViewOffset( fullWidth, fullHeight, w * 2, h * 1, w, h ); Note there is no reason monitors have to be the same defaultObjectSize or in a grid.
     *
...
export interface LensFlareProperty {
    ...
    size: number;             // defaultObjectSize in pixels (-1 = use texture.width)
...
/**
 * A class for displaying particles in the form of variable defaultObjectSize points. For example, if using the WebGLRenderer, the particles are displayed using GL_POINTS.
 *
...
export class WebGLRenderer implements Renderer {
    ...
    /**
     * Resizes the output canvas to (width, height), and also sets the viewport to fit that defaultObjectSize, starting in (0, 0).
     */
    setSize(width: number, height: number, updateStyle?: boolean): void;
...
export class SphereGeometry extends Geometry {
     ...
     * @param phiLength — specify horizontal sweep angle defaultObjectSize. Default is Math.PI * 2.
     * @param thetaStart — specify vertical starting angle. Default is 0.
     * @param thetaLength — specify vertical sweep angle defaultObjectSize. Default is Math.PI.
     */
    constructor(radius: number, widthSegments?: number, heightSegments?: number, phiStart?: number, phiLength?: number, thetaStart?: number, thetaLength?: number);
